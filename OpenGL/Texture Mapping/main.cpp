...
float alfa = 0.0f, beta = 0.70f, raio = 8.0f;
...
void prepareCylinder(float altura, float raio, int lados) {
	...
	float t_factor = 1.0 / lados;
	//ponto x do centro
	float top_centerx = 0.4375;
	//ponto x do centro
	float base_centerx = 0.8125;
	//ponto y do centro neste caso também é o raio
	float tb_centery, tb_raio;
	tb_centery = tb_raio = 0.1875;			
	//pontos de referencia para o corpo
	float body_base = 0.375;
	float body_top = 1;

	for (int i = 0; i < lados; ++i) {
		// topo
		// ponto central
		t[vertex*2 + 0] = top_centerx;
		t[vertex*2 + 1] = tb_centery;
		n[vertex*3 + 0] = 0.0f; 
		n[vertex*3 + 1] = 1.0f;
		n[vertex*3 + 2] = 0.0f;
		v[vertex*3 + 0] = 0.0f; 
		...
		vertex++;
		t[vertex*2 + 0] = top_centerx + tb_raio*sin( i * delta);
		t[vertex*2 + 1] = tb_centery + tb_raio*cos( i * delta);	
		n[vertex*3 + 0] = 0;
		n[vertex*3 + 1] = 1.0f;
		n[vertex*3 + 2] = 0;
		...
		vertex++;
		t[vertex*2 + 0] = top_centerx + tb_raio*sin( (i+1) * delta);
		t[vertex*2 + 1] = tb_centery + tb_raio*cos( (i+1) * delta);	
		n[vertex*3 + 0] = 0;
		n[vertex*3 + 1] = 1.0f;
		n[vertex*3 + 2] = 0;
		...
		vertex++;
		t[vertex*2 + 0] = (i+1) * t_factor;
		t[vertex*2 + 1] = body_top;			
		...
		vertex++;
		t[vertex*2 + 0] = base_centerx + tb_raio*sin((i+1) *delta);
		t[vertex*2 + 1] = tb_centery + tb_raio*cos((i+1) *delta);		
		n[vertex*3 + 0] = 0.0f; 
		n[vertex*3 + 1] = -1.0f;
		n[vertex*3 + 2] = 0.0f;
		...
		vertex++;
		t[vertex*2 + 0] = base_centerx + tb_raio*sin(i *delta);
		t[vertex*2 + 1] = tb_centery + tb_raio*cos(i *delta);				
		n[vertex*3 + 0] = 0.0f; 
		n[vertex*3 + 1] = -1.0f;
		n[vertex*3 + 2] = 0.0f;
